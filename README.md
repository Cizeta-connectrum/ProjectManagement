...
`index.html` の手動分析とは別に、`scripts/auto-check.mjs` が GitHub Actions で **5分おきを目安に**
自動実行され、監視の密度に濃淡をつけています。

> **注意**: GitHub Actionsのスケジュール実行（cron）は、指定した時刻通りに発火する保証がありません。
> 負荷状況によっては数十分〜1時間以上ずれることがあります。そのため本スクリプトは「壁時計の分」ではなく
> 「前回の本チェックからの経過時間（25分以上）」で判定しており、実際の発火がどれだけずれても
> 価格データの記録や分析が取りこぼされないようにしています。ただし「ポジション保有中は5分おきに判断する」
> という集中監視も同じcronに依存しているため、実際にはGitHub側の遅延によりそれより粗い間隔（状況により
> 1時間近く空くこともあります）でしか判断できないことがあります。
...
- **ポジション保有中の銘柄（集中監視）**: エントリー通知が出た銘柄は、解決するまで実行のたびに毎回（本来は5分おき、
  実際はGitHub側の遅延に左右されます）Claudeへ「保有継続 / TP到達 / SL到達 / 撤退推奨」を判断させます。
  TP到達・SL到達・撤退推奨のいずれかになったらDiscordに通知して追跡を終了し、通常監視に戻ります
...
- 実行状態は `data/open-positions.json`（保有中ポジション）と `data/state.json`（前回の本チェック時刻）に
  JSONで保存され、実行のたびにワークフローが自動でリポジトリにコミットします（GitHub Actionsの実行環境は
  毎回使い捨てのため、状態をリポジトリ側に永続化しています）
...
- **GitHub Actions**: 5分おきを指定していますが、実際の発火回数はGitHub側の遅延・間引きに左右されるため
  読みにくく、想定（1日288回）より少なくなることが多いです。いずれにせよポジション未保有時はほぼ即終了するため、
  無料枠（プライベートリポジトリで月2,000分）に収まらない場合でも増加分はごく少額（数百円程度）の見込みです
